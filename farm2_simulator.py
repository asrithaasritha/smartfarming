...
SOLUTION_URL = "http://127.0.0.1:5000/get_solution/farm2"
...
while True:
    ...
    if response.status_code == 200:
        print(f"🌱 [Farm 2] Sent data: {farm_data}")
    else:
        print(f"❌ [Farm 2] Failed to send data: {response.text}")
    ...
    if solution_response.status_code == 200:
        ...
        print(f"✅ [Farm 2] Received Solution: {solution_data['solution']}")
    else:
        print(f"❌ [Farm 2] Failed to fetch solution")
